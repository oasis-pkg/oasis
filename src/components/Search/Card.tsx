...
const Card: Component<{ data: pkg }> = ({ data }) => {
	...
	return (
		<A
			...
			class='flex gap-x-8 px-8 py-4 items-center rounded-md bg-gray-100 shadow-md hover:shadow-lg ease-in-out duration-200'>
			<PkgIcon class='w-10 h-10 fill-slate-900' platform={data.platform} />
			<div class='flex grow flex-col gap-y-2'>
				<div class='flex items-baseline gap-x-3'>
					<p class=' text-slate-900 text-lg font-medium select-all font-heebo'>{data.name}</p>
										...
										<a href={i.url} target='_blank'>
											<Switch fallback={<Earth class='w-10 h-10 fill-slate-700' />}>
												<Match when={i.kind == 'github'}>
													<SiGithub class='w-8 h-8 fill-slate-700' />
												</Match>
